...
## 2.1 将字符串转换为可度量值 
search_engine_indexer = StringIndexer(inputCol="Platform", outputCol="s").fit(df)    # 返回对应的模型,即StringIndexerModel
...
## 对使用的搜索引擎独热编码
search_engine_encoder = OneHotEncoder(inputCol df = search_engine_encoder.transform(df)
