...
def background_thread():
    ...
    while True:
        ...
        intrusion_count = redis_connection.zscore(
            "count-intrusion", "intrusion-count")
        normal_count = redis_connection.zscore("count-normal", "normal-count")
        host_ranking = redis_connection.zrevrange(
            "count-host", 0, 10, withscores=True)
        req_ranking = redis_connection.zrevrange(
            "count-req-url", 0, 10, withscores=True)
        ...
        statuscode_raw = redis_connection.zrange(
            "count-statuscode", 0, -1, withscores=True)
        req_method_raw = redis_connection.zrange(
            "count-req-method", 0, -1, withscores=True)
        datetime_raw = redis_connection.zrange(
            "count-datetime", 0, -1, withscores=True)
        ...
        i_n_datetime_sorted_ts = sorted(intrusion_container)
        ...
        intrusion_set = eval(bytes.decode(
            intrusion_pubsub.parse_response()[2]))
        ...
        for item in intrusion_set:
            if type(item) is not dict:
                item = eval(item)
            URl = 'http://api.map.baidu.com/location/ip?ak=RNeT2wMdQiyB2lssimPBW9N2jdIL6jBt&ip=' + \
                item['host'] + '&coor=gcj02'
        ...
        for item in intrusion_scatter:
            redis_connection.lpush("intrusion-scatter-all", str(item))
        ...
        for item in normal_scatter:
            redis_connection.lpush("normal-scatter-all", str(item))

        # History Tracking - Map scatter info
        # Bytes to json
        raw_intrusion_scatter_all_set = redis_connection.lrange(
            "intrusion-scatter-all", 0, 1000)
        raw_normal_scatter_all_set = redis_connection.lrange(
            "normal-scatter-all", 0, 1000)
        ...
        for i in statuscode_raw:
            statuscode_set.append(
                {"value": int(i[1]), "name": bytes.decode(i[0])})
            statuscode_legend.append(bytes.decode(i[0]))
        # Req method pie
        req_method_set = []
        req_method_legend = []
        for i in req_method_raw:
            req_method_set.append(
                {"value": int(i[1]), "name": bytes.decode(i[0])})
        ...
        for i in datetime_sorted:
            ...
            datetime.append(time.strftime("%Y-%m-%d %H:%M:%S",
                                          time.localtime(int(float(i)))))
